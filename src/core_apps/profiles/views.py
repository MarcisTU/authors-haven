        ...
        try:
            ...
            if user_profile.check_following(profile):
                ...
                message = (f"Hi there, {profile.user.first_name}!!, the user "
                           f"{user_profile.user.first_name} {user_profile.user.last_name} is now following you.")
            ...
            return NotFound("You cant follow a profile that does not exist.")
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                ...
                "message": f"You can't unfollow {profile.user.first_name} {profile.user.last_name},"
                           f" since you are not following them in the first place.",
            }
            return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)
        else:
